...
**🔗 Live dashboard:** https://diegotddd-olist-analytics.streamlit.app
...
An interactive Streamlit dashboard presents the findings. It is deployed live (link above); screenshots below in case the app is asleep.

![Dashboard overview](images/dashboard-overview.png)

![Review score distribution](images/dashboard-scores.png)
...
- `fct_orders` — one row per order, with measures: order value, payment installments, item count, freight, review score, delivery days, delay days, and an `is_delayed` flag. The review score uses the most recent review per order (via a window function) to avoid averaging multiple reviews into fractional scores.
